 ...
 */
async function applyHighlighting(type, grammar, embeddedGrammar, sample, includeCommonGrammars) {
  const grammarJson = PARSERS[type](grammar);
  const embeddedGrammarJson = PARSERS[type](embeddedGrammar);
  ...
  embeddedGrammarJson.extensions ??= [];
  embeddedGrammarJson.names ??= [];
  embeddedGrammarJson.scopeName ??= embeddedGrammarJson.scope; // Set scope if it is under different key in source

  const grammars = includeCommonGrammars
    ? [...common, grammarJson, embeddedGrammarJson]
    : [grammarJson, embeddedGrammarJson];
...
export async function load() {
  ...
  const embeddedGrammarInput = $('#embedded-grammar').value;
  ...
  const embeddedGrammar = embeddedGrammarInput;
  const sample = sampleType.includes('text') ? sampleInput : await fetch(sampleInput).then((data) => data.text());
  let fileType = 'json';
  if (grammarType === 'url') {
    if (/\.(json|json-tmlanguage)$/i.test(grammarInput)) fileType = 'json';
    if (/\.(cson|cson-tmlanguage)$/i.test(grammarInput)) fileType = 'cson';
    else if (/\.(yaml|yaml-tmlanguage)$/i.test(grammarInput)) fileType = 'yaml';
    else if (/\.(xml|tmlanguage)$/i.test(grammarInput)) fileType = 'plist';
  } else {
    if (/<!doctype\s*plist/i.test(grammar)) fileType = 'plist';
    if (/^\s*\{/i.test(grammar)) fileType = 'json';
    else if (/^\w+:\s*\[\s*$/m.test(grammar)) fileType = 'cson';
    else if (/^\w+:\s*$/m.test(grammar)) fileType = 'yaml';
  }
  const highlightTree = await applyHighlighting(fileType, grammar, embeddedGrammar, sample, includeCommonGrammars);
